/*
  Script:      T_EWS_VERIFICATION_RB_VAL.sql
  Schema:      ACCOUNT
  Author:      Jyoti Jerath
  Date:        11/19/2020
  Purpose:     dropped table validation- T_LU_EWS_VERIFICATION_CHANNEL
*/

DECLARE
	v_table_name 	all_tables.TABLE_NAME%TYPE := 'T_LU_EWS_VERIFICATION_CHANNEL';
    v_sequence_name all_sequences.SEQUENCE_NAME%TYPE := 'SEQ_CHANNEL_ID';
    v_exists 		NUMBER := 0;
    v_err			NUMBER := 0;
BEGIN
    SELECT COUNT(1)
